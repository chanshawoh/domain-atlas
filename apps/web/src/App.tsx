...
function Confidence({ node }: { node: BusinessDomain }) {
  return <span className={'confidence ' + (node.confidence === 'low' ? 'low' : '')}>{node.confidence === 'low' && <AlertCircle />}{t(confidenceNames[node.confidence] + '置信度')}</span>;
}
...
function ProjectView({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  const recordsPanel = <section className="panel records-panel">
    ...
    <div className="record-list">{visibleChanges.map(record => <button key={record.id} className={'record-row ' + (selection?.id === record.id ? 'selected' : '')} onClick={() => openChange(record)}>
      <time>{date(record.recordedAt)}</time><span className="record-summary"><strong>{recordTitle(record)}</strong><small>{record.affectedCapabilityIds.length ? record.affectedCapabilityIds.map(capabilityName).join('、') : t("暂无关联能力")} · {t('{count} 个文件', { count: record.changedFiles.length })}{record.supersedes && t("· 关联历史记录")}</small></span><span className="badge">{t(kindNames[record.kind])}</span><Status record={record} />
    </button>)}</div>
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {error ? <div role="alert" className="error-panel"><AlertCircle /><div><strong>{t("项目数据读取失败")}</strong><p>{serviceMessage(error)}</p><button onClick={() => setRevision(value => value + 1)}>{t("重新加载")}</button></div></div> : loading ? <div className="loading" role="status"><RefreshCw className="spinning" />{t("正在读取项目事实与 Git 状态…")}</div> : data && <>
        {page !== 'guide' && <div className="metrics">{[{ label: t("业务域"), value: data.totals.domains }, { label: t("业务能力"), value: data.totals.capabilities }, { label: t("变更记录"), value: data.totals.changes, sub: t('已提交 {count}', { count: data.totals.committed }) }, { label: t("待提交"), value: data.totals.pending }].map(item => <div key={item.label}><span>{item.label}</span><div><strong>{item.value}</strong>{item.sub && <small>{item.sub}</small>}</div></div>)}</div>}
    ...
    {page !== 'guide' && selection && !error && !loading && <aside ref={detailRef} className="detail" aria-label={t("详情面板")}><div className="detail-heading"><span>{selection.kind === 'change' ? t("变更详情") : selection.kind === 'domain' ? t("业务域详情") : t("能力详情")}</span><button className="icon-button" aria-label={t("关闭详情")} onClick={() => setSelection(null)}><X /></button></div>
      ...
      <Section title={t("变更文件 ·") + selectedChange.changedFiles.length}>{!selectedChange.changedFiles.length && <p className="muted">{t("未记录文件变化")}</p>}{!selectedChange.fileChanges && <p className="muted">{t("该记录未保存文件版本")}</p>}<ul className="files">{selectedChange.changedFiles.map(file => { const transition = selectedChange.fileChanges?.find(item => item.path === file); return <li key={file}><div><FileCode2 /><code>{file}</code><span>{transition ? !transition.before ? t("新增") : !transition.after ? t("删除") : t("修改") : t("已记录")}</span></div>{transition && <details><summary>{t("文件版本")}</summary><code>{t("前：")}{transition.before ? transition.before.oid + ' / ' + transition.before.mode : t("不存在")}{'\n'}{t("后：")}{transition.after ? transition.after.oid + ' / ' + transition.after.mode : t("不存在")}</code></details>}</li>; })}</ul></Section>
