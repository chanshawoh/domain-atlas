...
const hostNames: Record<string, string> = { codex: 'Codex', cursor: 'Cursor' };
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {error ? <div role="alert" className="error-panel"><AlertCircle /><div><strong>{t("项目数据读取失败")}</strong><p>{serviceMessage(error)}</p><button onClick={() => setRevision(value => value + 1)}>{t("重新加载")}</button></div></div> : loading ? <div className="loading" role="status"><RefreshCw className="spinning" />{t("正在读取项目事实与 Git 状态…")}</div> : data && <>
        ...
        {page === 'guide' && <article className="panel guide"><div className="guide-intro"><BookOpen /><h2>{t("把开发过程，变成可追溯的业务事实")}</h2><p>{t("Web UI 在一个端口集中展示已登记项目，当前页面读取所选项目的事实文件。完成配置后，已接入宿主的任务会通过生命周期钩子留下变更记录。")}</p></div><Section title={t("01 · 初始化与业务基线")}><p>{t("全局安装 DomainAtlas 后，在目标 Git 项目根目录执行：")}</p><pre>domainatlas init{'\n'}domainatlas build{'\n'}domainatlas ui</pre><p>{t("build 从已跟踪源码构建结构基线。也可让 AI 使用 DomainAtlas skill 分析业务语义，再通过 build --input 导入。已有 Git 提交不会被自动回放为业务历史。")}</p></Section><Section title={t("02 · 接入宿主生命周期")}><p>{t("一次性安装用户级钩子：")}</p><pre>domainatlas init -g --codex{'\n'}domainatlas init -g --cursor</pre><p>{t("本仓库提供")}<code>.codex/hooks.json</code>{t(" 和 ")}<code>.cursor/hooks.json</code>{t("，分别将 Codex 与 Cursor 的轮次事件传给 CLI。在 Codex 中通过")}<code>/hooks</code>{t("，在 Cursor 中通过 Settings → Hooks")} {t("审阅并信任项目钩子。")}</p><p>{t("接入其他仓库时，将钩子命令指向已构建的 DomainAtlas CLI 的绝对路径；事实始终写入目标项目。")}</p></Section><Section title={t("03 · 打开本地工作台")}><pre>pnpm ui{'\n'}{t("# 自定义端口")}{'\n'}pnpm ui --port 4311</pre><p>{t("在任意目录运行")}<code>domainatlas ui</code>{t("，进入全部项目列表。旧版项目可通过")}<code>{t("domainatlas ui --scan /项目父目录")}</code> {t("批量登记。")}</p></Section><Section title={t("如何阅读记录")}><p>{t("“已提交”表示事实记录进入 Git 历史。“未采集”表示本条记录没有结构化测试结果。纠正、撤销通过引用保留原始历史。")}</p><p>{t("业务域和能力名称来自结构分析或路径推断，可在详情中核对来源与置信度。")}</p></Section></article>}
    ...
    <AnimatePresence>{page !== 'guide' && selection && !error && !loading && <motion.aside initial={{ opacity: reducedMotion ? 1 : 0, x: reducedMotion ? 0 : 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: reducedMotion ? 0 : 16 }} transition={{ duration: reducedMotion ? 0 : .18 }} ref={detailRef} className="detail" aria-label={t("详情面板")}><div className="detail-heading"><span>{selection.kind === 'change' ? t("变更详情") : selection.kind === 'domain' ? t("业务域详情") : t("能力详情")}</span><button className="icon-button" aria-label={t("关闭详情")} onClick={() => setSelection(null)}><X /></button></div>
      {selectedChange ? <><div className="detail-title"><h2>{recordTitle(selectedChange)}</h2><div className="badges"><span className="badge">{t(kindNames[selectedChange.kind])}</span><Status record={selectedChange} /></div><p>{date(selectedChange.recordedAt)} <span>{t("· 来源 {host}", { host: hostNames[selectedChange.source.host] ?? selectedChange.source.host })}</span></p></div>
      ...
      <Section title={t("任务来源")}><p>{hostNames[selectedChange.source.host] ?? selectedChange.source.host}</p><code>{selectedChange.source.taskId ?? t("未提供任务 ID")}</code></Section>
