...
const sourceName = (source: string) => source === 'codebase-memory-mcp' ? '图谱推导' : source === 'changed-file-path' ? '路径推断' : source === 'baseline-file-path' ? '基线 · 路径推断' : source === 'baseline-code-graph' ? '基线 · 图谱推导' : source === 'baseline-ai-analysis' ? '基线 · AI 业务分析' : source;
...
function ProjectView({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><aside className="sidebar"><nav aria-label="主导航"><button onClick={onBack}><ArrowLeft />全部项目</button>{([{ id: 'map', label: '业务地图', icon: Map }, { id: 'changes', label: '变更记录', icon: History }, { id: 'guide', label: '接入说明', icon: BookOpen }] as const).map(item => <button key={item.id} className={page === item.id ? 'active' : ''} aria-current={page === item.id ? 'page' : undefined} onClick={() => navigate(item.id)}><item.icon />{item.label}</button>)}</nav><div className="storage"><span>本地事实存储</span><code>.domainatlas/</code></div></aside>
    <main className="main">
      ...
      {error ? <div role="alert" className="error-panel"><AlertCircle /><div><strong>项目数据读取失败</strong><p>{error}</p><button onClick={() => setRevision(value => value + 1)}>重新加载</button></div></div> : loading ? <div className="loading" role="status"><RefreshCw className="spinning" />正在读取项目事实与 Git 状态…</div> : data && <>
        ...
        {page === 'map' && data.baseline && <div className="info-banner"><span>业务基线 · {date(data.baseline.recordedAt)} · 选取 {data.baseline.coverage.selectedFiles} 个证据文件{data.baseline.coverage.limited ? ' · 范围有限' : ''}。反映构建时的业务结构，后续变更单独记录。</span></div>}
        ...
        {page === 'guide' && <article className="panel guide"><div className="guide-intro"><BookOpen /><h2>把开发过程，变成可追溯的业务事实</h2><p>Web UI 在一个端口集中展示已登记项目，当前页面读取所选项目的事实文件。完成配置后，Codex 任务会通过生命周期钩子留下变更记录。</p></div><Section title="01 · 初始化与业务基线"><p>全局安装 DomainAtlas 后，在目标 Git 项目根目录执行：</p><pre>domainatlas init{'\n'}domainatlas build{'\n'}domainatlas ui</pre><p>build 从已跟踪源码构建结构基线。也可让 AI 使用 DomainAtlas skill 分析业务语义，再通过 build --input 导入。已有 Git 提交不会被自动回放为业务历史。</p></Section><Section title="02 · 接入 Codex 生命周期"><p>本仓库提供 <code>.codex/hooks.json</code>，将 UserPromptSubmit / Stop 事件传给 CLI。在 Codex 中通过 <code>/hooks</code> 审阅并信任项目钩子。</p><p>接入其他仓库时，将钩子命令指向已构建的 DomainAtlas CLI 的绝对路径；事实始终写入目标项目。详见仓库 <code>docs/codex-integration.md</code>。</p></Section><Section title="03 · 打开本地工作台"><pre>pnpm ui{'\n'}# 自定义端口{'\n'}pnpm ui --port 4311</pre><p>在任意目录运行 <code>domainatlas ui</code>，进入全部项目列表。旧版项目可通过 <code>domainatlas ui --scan /项目父目录</code> 批量登记。</p></Section><Section title="如何阅读记录"><p>“已提交”表示事实记录进入 Git 历史。“未采集”表示本条记录没有结构化测试结果。纠正、撤销通过引用保留原始历史。</p><p>业务域和能力名称来自结构分析或路径推断，可在详情中核对来源与置信度。</p></Section></article>}
