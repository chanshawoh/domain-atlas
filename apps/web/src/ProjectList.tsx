...
export function ProjectList({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><img src="/domainatlas.svg" width="23" height="23" alt="" /><span>DomainAtlas</span></div><span className="muted">{t("项目工作台")}</span><span className="project-meta badge">{t("本地优先")}</span><LanguageSwitcher /></header>
    <main className="project-workspace">
      ...
      {error ? <div role="alert" className="error-panel"><AlertCircle /><div><strong>{t("项目列表读取失败")}</strong><p>{serviceMessage(error)}</p><button onClick={() => setRevision(value => value + 1)}>{t("重新加载")}</button></div></div> : loading ? <div className="loading" role="status">{t("正在读取项目列表…")}</div> : <>
        ...
        </article>)}</div> : <section className="panel empty"><FolderTree /><strong>{query ? t("没有匹配的项目") : t("还没有登记的项目")}</strong><p>{query ? t("尝试其他项目名称或路径。") : t("在项目里执行 domainatlas init 后，刷新此页面即可看到项目。")}</p></section>}
